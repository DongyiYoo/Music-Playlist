...
public class PlaylistGUI extends javax.swing.JFrame {
    ...
    private void popaddBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_popaddBtnActionPerformed
        // TODO add your handling code here:
       /* if (!LikedSongGUI.stackInterface.isEmpty()) {
        String lastSongInfo = (String) LikedSongGUI.stackInterface.peek(); // 스택의 peek 메서드를 사용하여 마지막 노래 정보를 확인합니다.
        if (lastSongInfo.contains("genre:POP")) {
            popTa.append(lastSongInfo + "\n"); // POP 장르의 경우, popTa에 정보를 추가합니다.
        }
    } else {
        JOptionPane.showMessageDialog(this, "No songs in the liked list.", "Info", JOptionPane.INFORMATION_MESSAGE);
    } */
        
        if (!LikedSongGUI.popStack.isEmpty()) {
        String songInfo = (String) LikedSongGUI.popStack.peek(); // 팝 스택의 마지막 노래 정보를 확인
        popTa.append(songInfo + "\n");
    } else {
        JOptionPane.showMessageDialog(this, "No POP songs in the liked list.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    /* if (!LikedSongGUI.stackInterface.isEmpty()) {
         String lastSongInfo = (String) LikedSongGUI.stackInterface.peek(); // 스택의 peek 메서드를 사용하여 마지막 노래 정보를 확인합니다.
         if (lastSongInfo.contains("genre:ROCK")) {
             rockTa.append(lastSongInfo + "\n"); // ROCK 장르의 경우, rockTa에 정보를 추가합니다.
         }
     } else {
         JOptionPane.showMessageDialog(this, "No songs in the liked list.", "Info", JOptionPane.INFORMATION_MESSAGE);
     } */
    ...
    if (!LikedSongGUI.rockStack.isEmpty()) {
        String songInfo = (String) LikedSongGUI.rockStack.peek(); // 팝 스택의 마지막 노래 정보를 확인
        rockTa.append(songInfo + "\n");
    } else {
        JOptionPane.showMessageDialog(this, "No POP songs in the liked list.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void popmoveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_popmoveBtnActionPerformed
        // TODO add your handling code here:
          if (!popList.isEmpty()) {
        // DLList에서 마지막 항목을 가져옵니다. 이는 추가적인 메서드 구현을 요구합니다. 예: getLast()
        Object lastElement = popList.getLast();
        // 가져온 마지막 항목을 rockList에 추가
        rockList.add(rockList.size() + 1, lastElement);
        // popList에서 마지막 항목 제거
        popList.remove(popList.size());
        // 변경된 리스트를 JTextArea에 다시 표시합니다.
        updateTextAreaFromLists();
    }
...
private void updateTextAreaFromLists() {
    // popList와 rockList의 내용을 기반으로 JTextArea 내용을 업데이트하는 로직
    popTa.setText(popList.printList());
    rockTa.setText(rockList.printList());
}
}
